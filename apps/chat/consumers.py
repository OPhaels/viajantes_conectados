...
class ConsumidorChat(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await self.accept()
        logger.info(f'Conexão WebSocket estabelecida: {self.usuario.email} - {self.uuid_conversa}')
    ...
    async def processar_mensagem(self, dados):
        ...
        if mensagem:
            ...
            logger.info(f'Mensagem enviada: {self.usuario.email} -> {self.uuid_conversa}')
    ...
    def obter_historico_mensagens(self, limite=50):
        ...
        try:
            conversa = Conversa.objects.get(uuid=self.uuid_conversa)
            mensagens = conversa.mensagens.select_related('remetente').order_by('-data_envio')[:limite]
    ...
    def sanitizar_conteudo(self, conteudo):
        ...
        # Remover scripts
        conteudo_sanitizado = re.sub(r'<script[^>]*>.*?</script>', '', conteudo_sanitizado, flags=re.DOTALL)
        
        # Remover event handlers
        conteudo_sanitizado = re.sub(r'on\w+\s*=\s*["\'].*?["\']', '', conteudo_sanitizado)
