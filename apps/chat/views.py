...
def _esta_bloqueado(usuario, outro):
    ...
    return Bloqueio.objects.filter(
        Q(bloqueador=usuario, bloqueado=outro) | Q(bloqueador=outro, bloqueado=usuario)
    ).exists()
...
def view_lista_conversas(request):
    ...
    conversas = (
        Conversa.objects.filter(participantes=request.user, ativa=True)
        .annotate(
...
def view_conversa(request, uuid_conversa):
    ...
    if outro and not _sao_amigos_ativos(request.user, outro):
        messages.warning(
            request,
            _("Você e este usuário não são mais amigos. O chat está em modo leitura."),
        )
...
def view_iniciar_conversa(request, uuid_usuario):
    ...
    if not _sao_amigos_ativos(request.user, outro):
        messages.error(
            request, _("Você só pode conversar com pessoas da sua lista de amigos.")
        )
...
def view_enviar_mensagem(request, uuid_conversa):
    ...
    if outro and _esta_bloqueado(request.user, outro):
        return JsonResponse(
            {"erro": _("Não é possível enviar mensagens para este usuário.")},
            status=403,
        )

    if outro and not _sao_amigos_ativos(request.user, outro):
        return JsonResponse(
            {"erro": _("Vocês não são mais amigos. Não é possível enviar mensagens.")},
            status=403,
        )
...
def view_mensagens_json(request, uuid_conversa):
    ...
    if after_id > 0:
        ainda_existe = Mensagem.objects.filter(conversa=conversa, id=after_id).exists()
        if not ainda_existe:
            return JsonResponse({"evento": "conversa_limpa"})

    novas = list(
        Mensagem.objects.filter(conversa=conversa, id__gt=after_id)
        .select_related("remetente")
        .order_by("data_envio")
    )

    ids_para_marcar = [
        m.id for m in novas if not m.lida and m.remetente_id != request.user.id
    ]
    if ids_para_marcar:
...
def view_apagar_conversa(request, uuid_conversa):
    ...
    try:
        with transaction.atomic():
            conversa_locked = Conversa.objects.select_for_update().get(pk=conversa.pk)
            qtd, detalhes = Mensagem.objects.filter(conversa=conversa_locked).delete()
        ...
        return JsonResponse(
            {"erro": _("Não foi possível limpar a conversa.")}, status=500
        )
...
def view_nao_lidas_detalhes_json(request):
    ...
    mensagens_map = {
        ...
        for m in (
            Mensagem.objects.filter(id__in=ids_ultimas).select_related(
                "remetente", "conversa"
            )
        )
    }
...
def view_iniciar_conversa_viagem(request, uuid_usuario, uuid_plano):
    ...
    if not ja_enviado:
        destino = (
            plano.pais_destino.nome if plano.pais_destino else "Destino indefinido"
        )
        cidade = plano.cidade_destino or ""
        motivo = (
            plano.get_motivo_viagem_display()
            if hasattr(plano, "get_motivo_viagem_display")
            else ""
        )
        ...
        if plano.imagens_urls:
            ...
            imagem_url = (
                f"https://flagcdn.com/w640/{plano.pais_destino.codigo_iso.lower()}.png"
            )
        ...
        logger.info(
            "Card de viagem enviado: %s -> %s (plano %s)",
            request.user.email,
            outro_usuario.email,
            uuid_plano_str,
        )
