...
class LogAuditoriaAdmin(admin.ModelAdmin):
    ...
    list_display = ('timestamp', 'tipo_acao', 'usuario_email', 'ip_address', 'resultado', 'requer_investigacao')
    list_filter = ('tipo_acao', 'resultado', 'requer_investigacao', 'timestamp')
    search_fields = ('usuario_email', 'ip_address', 'endpoint')
    readonly_fields = ('id', 'timestamp', 'usuario_email', 'ip_address', 'user_agent', 
                      'metodo_http', 'codigo_resposta', 'endpoint', 'dados_alterados')
    ordering = ['-timestamp']
    ...
    fieldsets = (
        ...
        (_('Usuário'), {
            'fields': ('usuario', 'usuario_email')
        }),
        (_('Requisição'), {
            'fields': ('ip_address', 'user_agent', 'metodo_http', 'endpoint', 'codigo_resposta')
        }),
        (_('Dados Adicionais'), {
...
class TentativaLoginFalhadoAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('id', 'timestamp', 'email_tentativa', 'ip_address', 'user_agent', 'motivo', 'usuario')
    ordering = ['-timestamp']
    date_hierarchy = 'timestamp'
