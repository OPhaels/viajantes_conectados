...
class LogAuditoria(models.Model):
    ...
    tipo_acao = models.CharField(
        _("tipo de ação"), max_length=50, choices=TIPO_ACAO_CHOICES, db_index=True
    )
    ...
    resultado = models.BooleanField(_("sucesso"), default=True, db_index=True)
    ...
    def __str__(self):
        return (
            f"{self.get_tipo_acao_display()} - {self.usuario_email} - {self.timestamp}"
        )
...
class TentativaLoginFalhado(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.email_tentativa} - {self.get_motivo_display()} - {self.timestamp}"
        )
    ...
    def contar_tentativas_recentes(cls, email_ou_ip, minutos=60):
        ...
        if "@" in str(email_ou_ip):
            return cls.objects.filter(
                email_tentativa=email_ou_ip, timestamp__gte=tempo_limite
            ).count()

        # Se é um IP
        return cls.objects.filter(
            ip_address=email_ou_ip, timestamp__gte=tempo_limite
        ).count()
