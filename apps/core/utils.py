...
def validar_token_verificacao(token):
    ...
    try:
        from rest_framework_simplejwt.utils import datetime_to_epoch, datetime, timedelta
        ...
        if validated_token.get('email_verificacao') is not True:
            return False, None, 'Token inválido: não é um token de verificação de email.'
        ...
        mensagem_erro = 'Token expirado. Solicite um novo.' if 'expired' in str(e) else f'Token inválido: {str(e)}'
