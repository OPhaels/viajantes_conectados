    ...
    def __init__(self, *args, **kwargs):
        ...
        for name, form_field in self.fields.items():
            if isinstance(form_field.widget, forms.CheckboxInput):
                ...
                existing = form_field.widget.attrs.get("class", "")
                form_field.widget.attrs["class"] = f"{existing} form-control".strip()
        ...
        self.fields["pais_destino"].queryset = Pais.objects.filter(ativo=True).order_by(
            "nome"
        )
...
class CampoUrlsImagens(forms.Field):
    ...
    def to_python(self, value):
        if not value:
            return []
        linhas = [
            linha.strip() for linha in value.strip().splitlines() if linha.strip()
        ]
        return linhas
...
class FormularioBuscaViajantes(forms.Form):
    ...
    motivo_viagem = forms.ChoiceField(
        ...
        required=False,
        widget=forms.Select(attrs={"class": "form-control form-control-custom"}),
    ...
    duracao_minima = forms.IntegerField(
        ...
        widget=forms.NumberInput(
            attrs={"class": "form-control form-control-custom", "placeholder": "Dias"}
        ),
