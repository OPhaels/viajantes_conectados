...
def validar_url_imagem(url: str, restrito_dominios: bool = False) -> tuple[bool, str]:
    ...
    for dominio in DOMINIOS_BLOQUEADOS:
        if dominio in url_lower:
            return False, "URL de domínio não permitido."

    # Verifica se tem extensão de imagem (mais permissivo — aceita qualquer
    # HTTPS)
    ...
    if restrito_dominios:
        ...
        if not confiavel:
            return (
                False,
                "Use imagens de sites conhecidos (Unsplash, Imgur, Pexels, etc.)",
            )

    # Tenta verificar se a URL retorna uma imagem (HEAD request)
    try:
        # Validar esquema da URL para segurança
        ...
        resp = requests.head(
            url,
            headers={"User-Agent": "Mozilla/5.0 (compatible; ViajantesConectados/1.0)"},
            timeout=5,
        ...
        if not tem_extensao:
            return (
                False,
                "Não foi possível verificar a URL. Use uma URL direta de imagem.",
            )
    except Exception:
        if not tem_extensao:
