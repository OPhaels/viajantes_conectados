    ...
    if request.method == 'POST':
        ...
        try:
            with transaction.atomic():
                ...
                if pais_codigo_iso:
                    pais = Pais.objects.filter(codigo_iso__iexact=pais_codigo_iso).first()

                if not pais:
                    pais = Pais.objects.filter(nome__iexact=pais_nome).first()

                if not pais:
                    logger.info('Criando novo país: %s (%s)', pais_nome, pais_codigo_iso)
                    ...
                    messages.success(request, f'País "{pais_nome}" adicionado automaticamente ao sistema!')
                ...
                if form.is_valid():
                    ...
                    messages.success(request, f'Plano de viagem para {pais_nome} criado com sucesso!')
            ...
            messages.error(request, 'Erro ao salvar o plano de viagem. Tente novamente.')
            form = FormularioPlanoViagem(request.POST)
            return render(request, 'destinos/criar_plano.html', {
                'titulo': 'Criar Plano de Viagem',
                'formulario': form,
...
def view_buscar_viajantes(request):
    ...
    queryset_destinos = (
        PlanoViagem.objects
        .filter(ativo=True, viagem_concluida=False)
    ...
    if formulario.is_valid():
        ...
        if dados.get('pais_destino'):
            queryset_destinos = queryset_destinos.filter(pais_destino=dados['pais_destino'])
        ...
        if dados.get('motivo_viagem'):
            queryset_destinos = queryset_destinos.filter(motivo_viagem=dados['motivo_viagem'])
    ...
    return render(request, 'destinos/buscar.html', {
...
def view_listar_viajantes(request):
    ...
    if cidade:
        queryset_viajantes = queryset_viajantes.filter(cidade_residencia__icontains=cidade)
    ...
    return render(request, 'destinos/buscar_viajantes.html', {
...
def view_meus_planos(request):
    ...
    return render(request, 'destinos/meus_planos.html', {
...
def view_detalhes_plano(request, uuid):
    ...
    return render(request, 'destinos/detalhes_plano.html', {
        'plano': plano,
