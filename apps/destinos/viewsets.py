...
from .serializers import PaisSerializer, PlanoViagemSerializer, PlanoViagemListaSerializer
...
class PlanoViagemViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.action == 'list':
            ...
            return PlanoViagem.objects.filter(
                Q(nivel_privacidade='publico') |  # Públicos
                Q(usuario=user) |  # Seus próprios
                Q(nivel_privacidade='amigos', usuario_id__in=amigos_ids)  # Amigos
            ).select_related('usuario', 'pais_destino').distinct()
    ...
    def buscar(self, request):
        ...
        queryset = PlanoViagem.objects.filter(
            ativo=True,
            viagem_concluida=False,
            nivel_privacidade='publico'
        ).exclude(usuario=user).select_related('usuario', 'pais_destino')
    ...
    def marcar_concluida(self, request, uuid=None):
        ...
        return Response(
            {
                'mensagem': _('Viagem marcada como concluída'),
