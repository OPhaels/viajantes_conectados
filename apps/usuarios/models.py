...
class Usuario(AbstractUser):
    ...
    nome_completo = models.CharField(
        _("nome completo"), max_length=150, validators=[MinLengthValidator(3)]
    )
    ...
    telefone_regex = RegexValidator(
        regex=r"^\+?1?\d{9,15}$",
        message=_(
            'Telefone deve estar no formato: "+999999999". Até 15 dígitos permitidos.'
        ),
    )
    telefone = models.CharField(
        _("telefone"), validators=[telefone_regex], max_length=17, blank=True
    )

    # Localização
    pais_residencia = models.CharField(
        _("país de residência"), max_length=100, blank=True
    )
    cidade_residencia = models.CharField(
        _("cidade de residência"), max_length=100, blank=True
    )
    ...
    foto_perfil = models.ImageField(
        _("foto de perfil"), upload_to="fotos_perfil/%Y/%m/", blank=True, null=True
    )
    ...
    tentativas_login_falhas = models.IntegerField(
        _("tentativas de login falhas"), default=0
    )
    bloqueado_ate = models.DateTimeField(_("bloqueado até"), null=True, blank=True)
