...
def view_cadastro_usuario(request):
    ...
    return render(
        ...
        {"formulario": formulario, "titulo": _("Cadastro")},
    )
...
def view_login_usuario(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if usuario.esta_bloqueado():
                ...
                messages.error(
                    request,
                    _(
                        f"Conta temporariamente bloqueada. Tente novamente em {tempo_restante} minutos."
                    ),
                )
                return render(
                    request, "usuarios/login.html", {"formulario": formulario}
                )
        ...
        if formulario.is_valid():
            ...
            messages.success(
                request, _(f"Bem-vindo, {usuario_autenticado.get_nome_exibicao()}!")
            )
    ...
    return render(
        request, "usuarios/login.html", {"formulario": formulario, "titulo": _("Login")}
    )
...
def view_editar_perfil(request):
    ...
    return render(
        ...
        {"formulario": formulario, "titulo": _("Editar Perfil")},
    )
...
def view_perfil_usuario(request, uuid=None):
    ...
    sao_amigos = (
        Amizade.sao_amigos(request.user, usuario) if not e_proprio_perfil else False
    )
    ...
    if e_proprio_perfil:
        ...
        planos = PlanoViagem.objects.filter(usuario=usuario).order_by("-data_criacao")
    else:
        # Mostrar apenas planos públicos de outros usuários
        planos = PlanoViagem.objects.filter(
            usuario=usuario, nivel_privacidade="publico"
    ...
    contexto = {
        ...
        "amigos_count": Amizade.objects.filter(usuario1=usuario).count()
        + Amizade.objects.filter(usuario2=usuario).count(),
