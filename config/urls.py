...
def view_home(request):
    ...
    if request.user.is_authenticated:
        ...
        planos_recentes = (
            PlanoViagem.objects.filter(usuario=usuario)
        ...
        # Contagem de amigos
        amigos_count = (
            Amizade.objects.filter(usuario1=usuario).count()
        ...
        solicitacoes_pendentes = SolicitacaoAmizade.objects.filter(
            destinatario=usuario, status="pendente"
        ).count()

        # Viajantes com destinos similares (excluindo o próprio usuário e
        # amigos)
        destinos_usuario = PlanoViagem.objects.filter(usuario=usuario).values_list(
            "pais_destino", flat=True
        )

        amigos_ids = list(
            Amizade.objects.filter(usuario1=usuario).values_list("usuario2", flat=True)
        ) + list(
            Amizade.objects.filter(usuario2=usuario).values_list("usuario1", flat=True)
        )
...
urlpatterns = [
    ...
    # JWT
    path(
        "api-token-auth/token/", TokenObtainPairView.as_view(), name="token_obtain_pair"
    ),
